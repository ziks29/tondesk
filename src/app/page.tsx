...
Never include markdown or extra text outside JSON.`;
  ...
  const getSelectedModel = () => {
    if (typeof window !== "undefined") {
      return (
        localStorage.getItem("tondesk_ai_model") ||
  ...
  async function handleDeploy(event: FormEvent) {
    ...
    try {
      ...
      if (systemPrompt.trim() && systemPrompt.trim() !== DEFAULT_SYSTEM_PROMPT)
  ...
  const toggleBotStatus = async (botId: string, currentStatus: boolean) => {
    try {
      ...
      if (res.ok) {
        setMyBots(
          myBots.map((b) =>
            b.id === botId ? { ...b, isActive: !currentStatus } : b,
          ),
        );
      }
    } catch (e) {
  ...
  const saveBotEdits = async (botId: string) => {
    ...
    try {
      ...
      if (editSystemPrompt && editSystemPrompt !== DEFAULT_SYSTEM_PROMPT)
        formData.append("systemPrompt", editSystemPrompt);
      if (editWelcomeMessage)
        formData.append("welcomeMessage", editWelcomeMessage);
  ...
  return (
    <Page back={false}>
      <div className="fixed inset-0 overflow-hidden pointer-events-none -z-10">
        <div
          className={`absolute -top-[10%] -left-[10%] w-[40%] h-[40%] rounded-full blur-[120px] opacity-20 ${isDarkMode ? "bg-[#0088cc]" : "bg-[#0088cc]/30"}`}
        />
        <div
          className={`absolute -bottom-[10%] -right-[10%] w-[40%] h-[40%] rounded-full blur-[120px] opacity-20 ${isDarkMode ? "bg-purple-600" : "bg-purple-600/20"}`}
        />
      </div>
      <main className="mx-auto min-h-screen w-full max-w-none md:max-w-7xl px-0 py-6 md:px-8 md:py-12 lg:px-12 lg:py-16 relative">
        <div className="mb-8 space-y-6 px-0 md:px-1 lg:mb-12">
          ...
          <div className="space-y-6 text-center sm:text-left">
            ...
            >
              Deploy RAG-powered Telegram bots from your knowledge base in
              minutes.
            </p>
            {tgUser && (
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="space-y-2">
              ...
              >
                ...
                credits from this wallet instead of using TON inside the bot
                chat.
              </p>
              {walletAddress ? (
                <p
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                <input
                  ...
                  onChange={(event) =>
                    setCustomTopUpAmount(event.currentTarget.value)
                  }
        ...
        <div className="grid grid-cols-1 gap-0 md:gap-6 px-0 lg:grid-cols-12 lg:gap-8">
          <section
            className={`col-span-1 lg:col-span-5 rounded-[2.5rem] border p-4 md:p-8 lg:p-10 shadow-none md:shadow-2xl backdrop-blur-2xl transition-all h-fit ${
              isDarkMode
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2
                    className={`text-lg font-semibold px-1 ${isDarkMode ? "text-white" : "text-slate-800"}`}
                  >
                    Deploy New Bot
                  </h2>
                  <Link
                    href="/settings"
                    className="flex items-center gap-1.5 text-xs text-slate-400 hover:text-[#0088cc] transition-colors p-1"
                  >
                ...
                <form className="space-y-5" onSubmit={handleDeploy}>
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className={`w-full rounded-2xl border px-4 py-3 text-sm outline-none transition-all focus:border-[#0088cc] focus:ring-4 focus:ring-[#0088cc]/10 ${
                        isDarkMode
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      onChange={(event) =>
                        setKnowledgeBaseText(event.currentTarget.value)
                      }
                    />
                  </div>
                        ...
                        <div key={index} className="flex gap-2">
                          <div className="relative flex-1">
                            ...
                            <input
                              ...
                              className={`w-full rounded-xl border pl-9 pr-4 py-2.5 text-sm outline-none transition-all focus:border-[#0088cc] focus:ring-4 focus:ring-[#0088cc]/10 ${
                                isDarkMode
                          ...
                          {files.map((file, index) => (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div
                                  className={`truncate text-sm font-medium max-w-[200px] ${isDarkMode ? "text-slate-200" : "text-slate-700"}`}
                                >
                                  {file.name}
                                </div>
                              </div>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <p
                        className={`text-sm font-semibold mb-3 ${isDarkMode ? "text-slate-200" : "text-slate-700"}`}
                      >
                        Crawl Settings
                      </p>
                      <div className="space-y-3">
                  ...
                  >
                    ...
                    >
                      <span
                        className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${webSearchEnabled ? "translate-x-5" : "translate-x-0"}`}
                      />
                    </button>
                  </div>
                  ...
                  >
                    ...
                    >
                      Bot replies use credits from your connected wallet
                      balance.
                    </div>
                  </div>
                ...
                <p className="px-1 text-xs text-slate-400">
                  Provide your bot token and knowledge base. Your connected
                  wallet will be registered as the bot's owner.
                </p>
              </div>
            </div>
          </section>
            ...
            {isLoadingBots ? (
              ...
              <div className="space-y-4">
                {myBots.map((bot) => (
                  <div
                    key={bot.id}
                    className={`group rounded-[2.5rem] border p-4 md:p-6 transition-all hover:scale-[1.01] ${
                      isDarkMode
                  ...
                  >
                    <div className="flex flex-col lg:flex-row items-start justify-between gap-4">
                      <div className="space-y-1 flex-1">
                        <p
                          className={`text-sm font-bold font-mono ${isDarkMode ? "text-slate-200" : "text-slate-800"}`}
                        >
                          {bot.botUsername
                            ? `@${bot.botUsername}`
                      ...
                      <div className="flex flex-col items-start lg:items-end gap-2 w-full lg:w-auto">
                        ...
                        <div className="flex gap-2 text-[10px] text-slate-500 font-mono text-right">
                          ...
                          <span className="text-[#0088cc]">
                            {MODELS.find((m) => m.id === bot.aiModel)?.name ||
                              "Unknown"}
                          </span>
                        </div>
                        <div className="flex gap-2 flex-wrap">
                          <button
                            onClick={() =>
                              toggleBotStatus(bot.id, bot.isActive)
                            }
                    ...
                    {editingBotId === bot.id ? (
                      <div className="mt-4 space-y-3">
                        ...
                        <div className="grid gap-2">
                          {MODELS.map((model) => (
                            <button
                              ...
                              className={`flex items-center justify-between w-full rounded-xl border p-2 text-left text-xs transition-all ${
                            ...
                            >
                              <div>
                                <p
                                  className={`text-xs font-bold ${editAiModel === model.id ? "text-[#0088cc]" : isDarkMode ? "text-slate-200" : "text-slate-700"}`}
                                >
                                  {model.name}
                                </p>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEditWelcomeMessage(e.target.value)
                          }
                        />
                        ...
                        <div className="space-y-2">
                          {editUrls.map((url, index) => (
                            <div key={index} className="flex gap-2">
                              <input
                                ...
                                value={url}
                                onChange={(e) =>
                                  updateEditUrl(index, e.target.value)
                                }
                              />
                              {editUrls.length > 1 && (
                                <button
                                  ...
                                  className="rounded-xl px-2 py-1 text-slate-400 hover:text-red-500 transition-colors"
                                >
                                  <X className="w-4 h-4" />
                                </button>
                              )}
                          ...
                          <button
                            type="button"
                            onClick={addEditUrl}
                            className={`text-xs font-semibold transition-colors ${isDarkMode ? "text-[#0088cc] hover:text-[#00aaee]" : "text-[#0088cc] hover:text-[#00aaee]"}`}
                          >
                            + Add another URL
                          </button>
                        </div>
                        ...
                        <div className="space-y-2">
                          <div>
                            ...
                            <input
                              ...
                              className={`mt-1 w-full rounded-lg border px-3 py-2 text-xs transition-colors ${
                                isDarkMode
                                  ? "border-slate-700 bg-slate-900/50 text-white focus:border-[#0088cc] focus:outline-none"
                                  : "border-slate-200 bg-white text-slate-900 focus:border-[#0088cc] focus:outline-none"
                              }`}
                        ...
                        >
                          ...
                          >
                            <span
                              className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${editWebSearchEnabled ? "translate-x-4" : "translate-x-0"}`}
                            />
                          </button>
                        </div>
                        ...
                        <div className="flex gap-2">
                          <Button
                            ...
                            disabled={
                              isSavingEdit || editKbText.trim().length < 10
                            }
