...
export async function GET(request: Request) {
  try {
    ...
    // ⚡ Bolt Optimization: Using native raw SQL COUNT(DISTINCT) rather than Prisma's groupBy
    // to avoid pulling massive group records into Node.js memory.
    const botIds = bots.map((bot) => bot.id);
    const uniqueUserCounts: Record<string, number> = {};

    if (botIds.length > 0) {
      const rawCounts = await prisma.$queryRaw<Array<{ botId: string; count: bigint | number }>>`
