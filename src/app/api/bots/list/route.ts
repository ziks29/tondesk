...
export async function GET(request: Request) {
  try {
    ...
    // ⚡ Bolt: Use $queryRaw with COUNT(DISTINCT) to avoid fetching all groupings into Node.js memory
    const uniqueUserCounts: Record<string, number> = {};
    if (botIds.length > 0) {
      const uniqueUsersGroups = await prisma.$queryRaw<Array<{ botId: string; count: bigint }>>`
